...
def generate_launch_description():
    ...
    robot = LaunchArgument(
        name="robot", default_value="jackal", description="robot model type"
    )
    ...
    tm_modules = LaunchArgument(
        name="tm_modules", default_value="rviz_ui"  # TODO breaks launch if empty
    )
    world = LaunchArgument(
        name="world", default_value="map_empty", description="world to load"
    )
    ...
    env_d = LaunchArgument(
        name="env_d", default_value="10", description="space between environments"
    )

    def create_task_generator(
        headlessness, namespace: str, prefix: str, reference: typing.List[float]
    ):
            ...
            [
                launch.actions.IncludeLaunchDescription(
                    launch.launch_description_sources.PythonLaunchDescriptionSource(
                        os.path.join(
                            bringup_dir,
                            "launch/shared/entity_manager/entity_manager.launch.py",
                        )
                    ),
                    launch_arguments={
                        **entity_manager.dict,
                ...
                launch.actions.IncludeLaunchDescription(
                    ...
                    launch_arguments={
                        ...
                        **record_data_dir.dict,
                        "namespace": namespace,
                        "task_generator_namespace": namespace,
...
def snail_grid(d: float, initial=None):
    if initial is None:
        # Start at (60, 60) which is within map bounds (50.00, 50.00) to (81.28, 74.03)
        initial = (60, 60)
